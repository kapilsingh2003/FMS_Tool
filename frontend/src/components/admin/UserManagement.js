...
const UserManagement = () => {
    ...
    const getProjectUsers = (project) => {
        ...
        return project.participants.map(username => {
    ...
    const getAvailableUsersForProject = (project) => {
    ...
    const handleAddUsersToProject = (project) => {
        setSelectedProject(project);
        const available = getAvailableUsersForProject(project);
    ...
    const handleRemoveUserFromProject = (project, userData) => {
        setSelectedProject(project);
    ...
    const confirmAddUsers = async () => {
        try {
            const promises = selectedUsersToAdd.map(userId =>
                usersAPI.addUserToProject(userId, selectedProject.id)
            );
    ...
    const confirmRemoveUser = async () => {
        try {
            const result = await usersAPI.removeUserFromProject(userToRemove.username, selectedProject.id);
            if (result.success) {
    ...
    return (
        <Container maxWidth="lg">
            <Box sx={{ mt: 4, mb: 4 }}>
                    ...
                    <Box sx={{ display: 'flex', alignItems: 'center', mb: 2 }}>
                        ...
                        <Box sx={{ flexGrow: 1 }}>
                            ...
                            <Typography variant="body2" color="text.secondary">
                                Manage users within projects - add or remove users from specific projects
                            </Typography>
                        </Box>
                        <Button
                            variant="contained"
                ...
                <Card>
                    <CardContent>
                        <Typography variant="h6" gutterBottom sx={{ display: 'flex', alignItems: 'center' }}>
                            <Group sx={{ mr: 1 }} />
                            Projects and Assigned Users
                        </Typography>

                        {projects.length === 0 ? (
                            <Alert severity="info">
                                No projects found. Create a project first to manage users.
                            </Alert>
                        ) : (
                                    ...
                                    <Accordion key={project.id} sx={{ mb: 2 }}>
                                        <AccordionSummary expandIcon={<ExpandMore />}>
                                            <Box sx={{ display: 'flex', alignItems: 'center', width: '100%' }}>
                                                ...
                                                <Chip
                                                    label={`${projectUsers.length} members`}
                                                    size="small"
                                                    color="primary"
                                                    sx={{ mr: 2 }}
                                                />
                                                <Button
                                                    ...
                                                    onClick={(e) => {
                                                        e.stopPropagation();
                                                        handleAddUsersToProject(project);
                                                    }}
                                                    sx={{ mr: 1 }}
                                                >
                                                    Add Users
                                                    ...
                                                    <Table size="small">
                                                        ...
                                                        <TableBody>
                                                            {projectUsers.map((userData) => (
                                                                <TableRow key={`${project.id}-${userData.username}`}>
                                                                    <TableCell>
                                                                    ...
                                                                    <TableCell align="center">
                                                                        {userData.role !== USER_ROLES.ADMIN && (
                                                                            <Tooltip title="Remove from Project">
                                                                                <IconButton
                                                                                    onClick={() => handleRemoveUserFromProject(project, userData)}
                                                                                    size="small"
                                                                                    color="error"
                                                                                >
                                                                                    <PersonRemove />
                ...
                >
                    <DialogTitle>
                        Add Users to Project: {selectedProject?.title}
                    </DialogTitle>
                    <DialogContent>
                        <Box sx={{ mb: 2 }}>
                            <TextField
                            ...
                            <List>
                                {filteredAvailableUsers.map((userData) => (
                                    <ListItem key={userData.id} divider>
                                        <ListItemIcon>
                                            <Checkbox
                                                checked={selectedUsersToAdd.includes(userData.id)}
                ...
                >
                    ...
                    <DialogContent>
                        <Typography variant="body1" gutterBottom>
                            Are you sure you want to remove <strong>{userToRemove?.name}</strong> from the project <strong>{selectedProject?.title}</strong>?
                        </Typography>
                        <Alert severity="warning" sx={{ mt: 2 }}>
