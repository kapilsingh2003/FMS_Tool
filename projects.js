    ...
    child.on('exit', async (code) => {
      ...
      if (dfCount > 0) {
        ...
        if (dfDataStart !== -1 && dfDataEnd !== -1) {
          ...
          // Fully refresh key reviews to repopulate after mapping updates
          await refreshProjectData(projectId, dataFrames, branchModelsDict);
        }
      }
...
// Helper to process only new key reviews (not existing ones)
async function processOnlyNewKeyReviews(projectId, newDataFrames, branchModelsDict) {
  // Similar to refreshProjectData but only inserts new keys
  const newKeyMap = new Map();
  const groups = await Project.getGroups(projectId);
  let dfIndex = 0;
  ...
  for (let groupIndex = 0; groupIndex < branchModelsDict.branch.length; groupIndex++) {
    ...
    for (let modelIndex = 0; modelIndex < groupModels.length; modelIndex++) {
      if (dfIndex >= newDataFrames.length) continue;
  ...
  // Insert only keys that don't already exist
  const KeyReview = require('../models/KeyReview');
  for (const [key, reviews] of newKeyMap) {
    for (const newReview of reviews) {
      const keyName = newReview.key_name;
      const modelCombo = newReview.model_combination;

      // Check if key review already exists
      const existing = await executeQuery(
        `SELECT kr.* FROM \`Key_Reviews\` kr
         ...
         WHERE fk.key_name = ? AND g.name = ? AND g.project_id = ?
         LIMIT 1`,
        [keyName, newReview.group_name, projectId]
      );
