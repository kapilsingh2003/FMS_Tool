  ...
  try {
    ...
    if (groups && Array.isArray(groups) && groups.length > 0) {
      for (const groupData of groups) {
        ...
        // For 2-way-vs-2-way, transform frontend keys to backend keys
        // Frontend: target1, reference1, target2, reference2
        // Backend: target, reference1, reference2, reference3
        let normalizedBranches = branches;
        if (comparisonType === '2-way-vs-2-way' || comparisonType === '2-way vs 2-way') {
          normalizedBranches = {
            target: branches?.target1 || branches?.target,
            reference1: branches?.reference1,
            reference2: branches?.target2,
            reference3: branches?.reference2
          };
        }

        // Get branch IDs - handle both reference1 and reference for 2-way comparisons
        const targetBranchId = normalizedBranches?.target ? await getBranchIdByName(normalizedBranches.target) : null;
        const ref1BranchId = normalizedBranches?.reference1 || normalizedBranches?.reference ? await getBranchIdByName(normalizedBranches.reference1 || normalizedBranches.reference) : null;
        const ref2BranchId = normalizedBranches?.reference2 ? await getBranchIdByName(normalizedBranches.reference2) : null;
        const ref3BranchId = normalizedBranches?.reference3 ? await getBranchIdByName(normalizedBranches.reference3) : null;
    ...
    try {
      ...
      for (const group of completeProject.groups) {
        ...
        // 2-way-vs-2-way is treated like 4-way in terms of branch/model structure
        ...
        if (comparisonType === '3-way' || comparisonType === '4-way' || comparisonType === '2-way-vs-2-way' || comparisonType === '2-way vs 2-way') {
          if (group.ref2_branch_name || group.branches?.reference2) {
            branches.push(group.ref2_branch_name || group.branches?.reference2);
          }
        }

        if (comparisonType === '4-way' || comparisonType === '2-way-vs-2-way' || comparisonType === '2-way vs 2-way') {
        ...
        const modelRows = modelCombinations.map((row) => {
          const models = [];
          // Always push models in the same order as branches (target, ref1, ref2, ref3)
          if (row.target) models.push(row.target);
          if (branches.length >= 2 && (row.reference || row.reference1)) models.push(row.reference || row.reference1);
          if (branches.length >= 3 && row.reference2) models.push(row.reference2);
          if (branches.length >= 4 && row.reference3) models.push(row.reference3);
          return models;
        });
      ...
      console.log('=== PROJECT CREATION: branch_models_dict for project', project.project_id, '===');
      console.log('Groups comparison types:', completeProject.groups.map(g => `${g.name}: ${g.comparison_type}`).join(', '));

      // Check if we have mixed comparison types
      const has2WayVs2Way = completeProject.groups.some(g =>
        (g.comparison_type || g.comparisonType) === '2-way-vs-2-way' || (g.comparison_type || g.comparisonType) === '2-way vs 2-way'
      );
      const hasRegular = completeProject.groups.some(g => {
        const ct = g.comparison_type || g.comparisonType;
        return ct !== '2-way-vs-2-way' && ct !== '2-way vs 2-way';
      });

      if (has2WayVs2Way && hasRegular) {
        console.error('❌ ERROR: Project has MIXED comparison types (2-way-vs-2-way and regular). This is not currently supported.');
        return res.status(400).json({
          success: false,
          message: 'Cannot mix 2-way-vs-2-way groups with other comparison types (2-way, 3-way, 4-way) in the same project. Please create separate projects for different comparison types.'
        });
      }

      console.log('Branch structure (2D):', JSON.stringify(branchList2D, null, 2));
      console.log('Model structure (3D):', JSON.stringify(modelList3D, null, 2));
      console.log('Complete dict:', JSON.stringify(branchModelsDict, null, 2));
      ...
      if (models && Array.isArray(models)) {
        for (const modelData of models) {
          ...
          if (typeof modelData === 'string') {
            ...
            normalized.ref1 = modelData.reference || modelData.reference1 || modelData.ref1 || null;

            // Handle 2-way-vs-2-way vs regular 3-way/4-way
            if (modelData.target2 && !modelData.reference3) {
              // 2-way-vs-2-way mode
              normalized.ref2 = modelData.target2 || null;
              normalized.ref3 = modelData.reference2 || null;
            } else {
...
async function refreshProjectDataAfterConfigUpdate(projectId) {
  ...
  try {
    ...
    child.on('exit', async (code) => {
      ...
      console.log('=== Python Output (first 2000 chars) ===');
      console.log(output.substring(0, 2000));
      console.log('=== End Python Output ===');

      if (output.length > 2000) {
        console.log('=== Python Output (last 1000 chars) ===');
        console.log(output.substring(output.length - 1000));
        console.log('=== End Last Output ===');
      }
      ...
      if (dfCount > 0) {
        ...
        if (dfDataStart !== -1 && dfDataEnd !== -1) {
          const dfDataSection = output.substring(dfDataStart, dfDataEnd);
          console.log('=== DF Data Section (first 1000 chars) ===');
          console.log(dfDataSection.substring(0, 1000));
          console.log('=== End DF Data Section ===');

          // Use [\s\S] instead of . to match newlines, and make it non-greedy with *?
          const dfMatches = dfDataSection.match(/DF_START (\d+)\n([\s\S]*?)\nDF_END \1/g);

          console.log(`Found ${dfMatches ? dfMatches.length : 0} DataFrame matches`);

          // Try alternative approach: manually find DF_START/DF_END pairs
          if (!dfMatches || dfMatches.length === 0) {
            console.log('Trying manual parsing...');
            const manualMatches = [];
            let startIdx = 0;
            while (true) {
              const startPos = dfDataSection.indexOf('DF_START ', startIdx);
              if (startPos === -1) break;

              const endMarkerPos = dfDataSection.indexOf('\nDF_END ', startPos);
              if (endMarkerPos === -1) break;

              const chunk = dfDataSection.substring(startPos, endMarkerPos);
              manualMatches.push(chunk);
              console.log(`Manual match found: starts at ${startPos}, ends at ${endMarkerPos}`);
              startIdx = endMarkerPos + 1;
            }
            console.log(`Manual parsing found ${manualMatches.length} chunks`);

            // Use manual matches as fallback
            if (manualMatches.length > 0) {
              for (let i = 0; i < manualMatches.length; i++) {
                const chunk = manualMatches[i];
                const lines = chunk.split('\n');
                // First line is "DF_START X", skip it and get JSON from remaining lines
                const jsonData = lines.slice(1).join('\n').trim();

                try {
                  const dfData = JSON.parse(jsonData);
                  dataFrames[i] = dfData;
                  console.log(`Manually parsed DataFrame ${i}: ${dfData.length} rows`);
                } catch (parseErr) {
                  console.error(`Error manually parsing DataFrame ${i}:`, parseErr.message);
                  console.error('JSON data (first 200 chars):', jsonData.substring(0, 200));
                }
              }
            }
          } else if (dfMatches) {
...
async function processEditConfigSync(projectId, newDataFrames, branchModelsDict) {
  ...
  // Also save comments before deletion (since DELETE cascades to Comments table)
  const savedComments = new Map(); // key: "fms_key_name|group_name|model_combo", value: array of comment objects
  ...
  for (const review of existingReviews) {
    ...
    // Fetch and save comments for this review
    if (review.key_review_id) {
      const comments = await executeQuery(
        'SELECT * FROM `Comments` WHERE key_review_id = ? ORDER BY created_at ASC',
        [review.key_review_id]
      );
      if (comments.length > 0) {
        savedComments.set(lookupKey, comments);
      }
    }

    // Debug: Log first few entries to verify data is being saved
    if (savedUserData.size <= 3) {
      console.log(`💾 Saved: ${lookupKey} -> status=${review.status}, kona=${review.kona_ids}, cl=${review.cl_numbers}, comments=${savedComments.has(lookupKey) ? savedComments.get(lookupKey).length : 0}`);
    }
  }

  console.log(`Saved ${savedUserData.size} existing key reviews with user data and ${savedComments.size} reviews with comments`);
  ...
  for (let groupIndex = 0; groupIndex < branchModelsDict.branch.length; groupIndex++) {
    ...
    for (let modelIndex = 0; modelIndex < groupModels.length; modelIndex++) {
      ...
      for (let rowIndex = 0; rowIndex < df.length; rowIndex++) {
        ...
        if (saved) {
          console.log(`📦 Restoring data for ${keyName}: status=${saved.status}, kona=${saved.kona_ids}, cl=${saved.cl_numbers}`);
        }
        ...
        await KeyReview.create({
          ...
          kona: saved ? saved.kona_ids : null,
          cl: saved ? saved.cl_numbers : null
        });
  ...
  // Step 4: Restore comments for matching key+model combinations from saved data
  const Comment = require('../models/Comment');
  const newReviews = await KeyReview.getByProject(projectId);
  let commentsRestoredCount = 0;
  ...
  for (const newReview of newReviews) {
    ...
    // Check if we have saved comments for this key+model combo
    if (savedComments.has(lookupKey)) {
      const commentsToRestore = savedComments.get(lookupKey);

      for (const oldComment of commentsToRestore) {
        await executeQuery(
      ...
      commentsRestoredCount++;
      console.log(`💬 Restored ${commentsToRestore.length} comment(s) for ${newReview.key_name}`);
    }
  }

  console.log(`✓ Comments restored for ${commentsRestoredCount} key reviews`);
}
